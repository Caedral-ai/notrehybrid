...
is allowed to make: **transfer ΔMSE with the collision cache**, under the
...
Do not open a `feat/cache` branch before Gate A is written as GO B.
...
2. **Kill hard at Gate B.** No tech report if ΔMSE ≤ 0 after a short K/τ sweep.
3. **Do not skip gates.** No Qwen, no cache training, no paper scaffolding
   before A then B.
...
6. **Do not pre-create Week 1+ modules** (`notre_linear.py`, `convert/`,
   `eval/mqar.py`) until that week starts.
...
- Fill [docs/decisions/gate-A.md](docs/decisions/gate-A.md) and
  [gate-B.md](docs/decisions/gate-B.md) when a run finishes, not before.
